...
<body>
	...
	<div class="row">
		...
		<div class="col-md-9">
	<input type="text" style="width:200px;" class="form-control" value="Lost Sim Card" readonly/>
									</div>
...
<div class="panel panel-default">
		...
		<div class="col-md-9">
			...
			$print = mysqli_query($mysql,"select tid from history where tid='$tid'");
...
<div class="panel-body">
			...
			$tid=$_POST["tid"];
			$mysql=mysqli_connect("localhost","simuneti_meghana","Projectsuraksha@2017","simuneti_suraksha") or die("Can't connect to DB");
			$print = mysqli_query($mysql,"select date,tid,Name,Case_Type from history where tid='$tid'");
			while($row = mysqli_fetch_array($print))
    				{
    				$name =$row['Name'];
    				$type=$row['Case_Type'];
			...
			$res=mysqli_query($mysql,"select * from lostsim where tid='$tid'");
			$res1=mysqli_query($mysql,"select * from lostsim1 where tid='$tid'");
			$res2=mysqli_query($mysql,"select * from lostsim2 where tid='$tid'");
			$res3=mysqli_query($mysql,"select * from lostsim4 where tid='$tid'");
...
echo "<a href=\"generatepdf.php\">Download PDF</a>";
echo "</div>";
}
else{
