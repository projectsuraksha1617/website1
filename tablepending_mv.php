...
echo "<script type=\"text/javascript\" src=\"clonetable.js\"></script>";
	function get_products()
	{
		...
		$data=mysqli_query($conn,"select * from vehicletheft1");
		$vehicletheft1=array();
		while($object= mysqli_fetch_object($data))
		{
		   $vehicletheft1[]=$object;
		}
		mysqli_close($conn);
		return $vehicletheft1;
	}
		echo "<form action=\"insert_active_mv.php\" method=\"post\">"; 
		echo "<table style=\"width:70%;margin-left:80px;\" class=\"table table-striped table-bordered table-hover\">";
		$vehicletheft1=get_products();
		$i=1;
		echo "<thead>";
